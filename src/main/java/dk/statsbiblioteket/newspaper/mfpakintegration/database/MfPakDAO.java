...
public class MfPakDAO {
    ...
    public static final String GET_EVENTS
            = "SELECT name, batchstatus.created FROM batchstatus, status WHERE batchstatus.statusrowId=status.rowId AND batchstatus.batchrowId=?";
    public static final String GET_ALL_BARCODES = "SELECT  batchId, rowId FROM batch";
    public static final String GET_ALL_EVENTS
            = "SELECT batchrowId, name, batchstatus.created FROM batchstatus, status WHERE statusrowId = status.rowId ";
    public static final String GET_NEWSPAPER_ID
            = "SELECT NewsPaperId FROM NewsPaper WHERE RowId = (SELECT NewsPaperRowId FROM Batch WHERE BatchId = ?) ";
    ...
    public MfPakDAO(MfPakConfiguration configuration) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Unable to find database driver '" + configuration.getDatabaseDriver() +
                    "' in classpath.", e);
        }
    }
    ...
    public Batch getBatchByBarcode(String barcode) throws SQLException {
        try (Connection con = getConnection();
             PreparedStatement stmt = con.prepareStatement(GET_BATCH_ID)) {
    ...
    public String getNewspaperID(String barcode) throws SQLException, InconsistentDatabaseException {
        try (Connection con = getConnection();
             PreparedStatement stmt = con.prepareStatement(GET_NEWSPAPER_ID)) {
    ...
    public NewspaperEntity getNewspaperEntity(String newspaperID, Date date) throws
        ...
        final String selectSql
                = "SELECT Name, PublicationLocation, FromDate, ToDate FROM NewsPaperTitle" + " WHERE NewsPaperRowId = (SELECT RowId FROM NewsPaper WHERE NewsPaperId = ?)" + " AND FromDate <= ?" + " AND ToDate >= ?";
                ...
                if (newspaperEntityExists) {
                    ...
                    if (rs.next()) {
                        throw new InconsistentDatabaseException(
                                "Found more than one newspaper entity for newspaperID '" + newspaperID + "' on date '" + date + "'");
                    }
                } else {
    ...
    public List<NewspaperDateRange> getBatchDateRanges(String batchID) throws SQLException {
        final String selectSql
                = "SELECT FromDate, ToDate FROM Film" + " WHERE BatchRowId = (SELECT RowId FROM Batch WHERE BatchId = ?)" + " ORDER BY FromDate ASC";
    ...
    public List<NewspaperEntity> getBatchNewspaperEntities(String batchID) throws SQLException {
        ...
        final String selectSql
                = "SELECT Name, FromDate, ToDate, PublicationLocation, NewsPaper.NewsPaperId" + " FROM NewsPaperTitle" + " JOIN NewsPaper ON NewsPaperTitle.NewsPaperRowId = NewsPaper.RowId" + " WHERE NewsPaperRowId = (SELECT NewsPaperRowId FROM Batch WHERE BatchId = ?)" + " AND (ToDate IS NULL OR ToDate >= (SELECT FromDate FROM Film" + " WHERE BatchRowId = (SELECT RowId FROM Batch WHERE BatchId = ?)" + " ORDER BY FromDate ASC LIMIT 1))" + " AND FromDate <= (SELECT ToDate FROM Film" + " WHERE BatchRowId = (SELECT RowId FROM Batch WHERE BatchId = ?)" + " ORDER BY ToDate DESC LIMIT 1)" + " ORDER BY FromDate ASC";
    ...
    public NewspaperBatchOptions getBatchOptions(String batchID) throws SQLException, InconsistentDatabaseException {
        ...
        final String selectSql
                = "SELECT OptionB1, OptionB2, OptionB3, OptionB4," + " OptionB5, OptionB6, OptionB7, OptionB8, OptionB9 FROM OrderLine" + " WHERE RowId = (" + " SELECT OrderLineRowId FROM OrderBatch" + " WHERE BatchRowID = (" + " SELECT RowId FROM Batch WHERE BatchId = ?))";
        ...
        try (Connection con = getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                if (!newspaperOptionsExists) {
                    ...
                    if (rs.next()) {
                        throw new InconsistentDatabaseException(
                                "Found more than one set of batch options for batch '" + batchID + "'");
                    }
                }
            }
        }
    ...
    public Date getBatchShipmentDate(String batchID) throws SQLException {
        ...
        final String selectSql
                = "SELECT Created FROM BatchStatus" + " WHERE BatchRowId = (SELECT RowId FROM Batch" + " WHERE BatchId = ?)" + " AND StatusRowId = (SELECT RowId FROM Status" + " WHERE Name = 'Batch shipped to supplier');";
